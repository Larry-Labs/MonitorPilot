...
- 切换后验证：自动读回输入状态，检测无信号端口；若显示器固件自动回退则静默同步 UI
...
前端共 37 个测试（App 生命周期/切换/轮询/重命名、MonitorCard 交互、类型定义），后端共 20 个测试（输入映射、配置持久化、m1ddc 解析；其中 6 个 macOS 专属解析测试仅在 macOS 上编译运行）。
